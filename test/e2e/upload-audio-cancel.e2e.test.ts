...
describe.each(TARGETS)('POST /api/v1/audio/upload/cancel/:jobId (e2e) — $label', ({ chatId }) => {
  // job "ocupante" (A): sobe de verdade, segura a única vaga de concorrência
  // da fila de áudio (UPLOAD_CONCURRENCY_LIMIT=1, aplicado à fila de áudio
  // independentemente da de vídeo) enquanto o caso 1 cancela um job atrás
  // dele na fila. É o único job desse arquivo que completa normalmente (sem
  // ser cancelado) — por isso é o único que precisa de limpeza no afterAll.
  ...
  it('cancels a job that is still queued behind an active upload', async () => {
    ...
    const cancelRes = await authed(request(app).post(`/api/v1/audio/upload/cancel/${queuedJobId}`));
  ...
  it('soft-cancels an upload already in progress: deletes the message instead of completing the job', async () => {
    ...
    const cancelRes = await authed(request(app).post(`/api/v1/audio/upload/cancel/${jobId}`));
    expect(cancelRes.status).toBe(200);
    ...
    if (settled.message_id) {
      const streamRes = await authed(request(app).get(`/api/v1/audio/stream/${chatId}/${settled.message_id}`));
