...
- Tests of `src/routes/**/route.ts` and `src/server.ts` mock the higher-level `src/telegram-client.ts` module instead (`jest.mock('@/telegram-client')`), stubbing its exported functions (`getVideoMessage`, `listVideos`, `listAllVideos`, `listChannels`, `getChannelVideos`) and the `client._media.getFile(dcId, location, offset, limit, signal)` path used by streaming.
