import { client, getAudioMessage } from '@/telegram-client';
import { buildContentDisposition, parseRange, SAFE_AUDIO_MIME_TYPE } from '@/utils/http-response';
import type { Request, Response } from 'express';
import { iterTelegramDocumentRange } from '../videos/telegram-range';

type StreamDisposition = 'inline' | 'attachment';

export async function streamTelegramAudio(
  req: Request,
  res: Response,
  ...
  try {
    const { document, size, mimeType, fileName } = await getAudioMessage(req.params.chatId, req.params.messageId);
    ...
    const safeMimeType = SAFE_AUDIO_MIME_TYPE.test(mimeType) ? mimeType : 'application/octet-stream';
