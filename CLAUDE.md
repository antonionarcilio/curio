...
It runs under a **separate Jest config**, `jest.e2e.config.js` (`testMatch: ['**/test/e2e/**/*.e2e.test.ts']`), for reasons that make it unsafe to share `jest.config.js`: it deliberately has **no `setupFiles`** — loading `test/setup-env.ts`'s fake credentials would make the tests authenticate against a nonexistent account instead of the real one in `.env` — it forces `maxWorkers: 1` because the tests share one real Telegram account and running them in parallel triggers `FLOOD_WAIT` (running two `test:e2e` invocations concurrently is just as unsafe — they'd race on the same account **and** on the shared `test/e2e/.state/fixtures.json`, so never run more than one at a time), and it sets a custom `testSequencer` (see below), a generous `testTimeout` (`1_200_000`ms) to cover the large fixture's upload time, and `forceExit: true` — even after `client.destroy()` (see below) stops GramJS's internal ping loop, each `TelegramClient` still holds connections to more than one datacenter (main + the one used for the large fixture's upload/download), and each schedules its own disconnect-confirmation timer that can fire a second or two after `destroy()` already resolved; `forceExit` stops the process as soon as results are reported instead of waiting on those.
...
**The 9 files form one strict chain**, not independent tests: `upload-video.e2e.test.ts` → `update-video.e2e.test.ts` → `videos-grouped.e2e.test.ts` → `list-channels.e2e.test.ts` → `videos-by-chat.e2e.test.ts` → `stream-video.e2e.test.ts` → `download-video.e2e.test.ts` → `purge-cache.e2e.test.ts` → `delete-video.e2e.test.ts` (mirroring `/api/v1/videos/grouped`, `/api/v1/channels`, `/api/v1/videos/by/:chatId`, `/api/v1/video/stream/...`, `/api/v1/video/dl/...`, `/api/v1/video/upload/...`, `/api/v1/video/update/...`, `/api/v1/video/delete/...`, `/api/v1/cache/purge` respectively). `purge-cache` runs after every read has exercised the caches and before the fixture is destroyed — it proves a read still works right after a purge, not just that the endpoint returns `200`.

Jest doesn't guarantee file execution order, so `test/e2e/sequencer.js` (plain CommonJS, outside the `ts-jest` pipeline) extends `@jest/test-sequencer`'s default `Sequencer` and sorts by an explicit priority list matching the chain above; anything not in the list falls to the end, alphabetically, instead of breaking. Each test file also still runs in its own isolated module registry, so the uploaded `messageId` can't just live in a module-level variable — `test/e2e/helpers/shared-state.ts` persists it per `chatId` to `test/e2e/.state/fixtures.json` (gitignored): `upload-video.e2e.test.ts` writes it, the middle files read it, `delete-video.e2e.test.ts` clears it. A file that can't find state for its `chatId` fails loudly (`upload-video.e2e.test.ts` precisa rodar antes) instead of silently skipping.

**Only `upload-video.e2e.test.ts` and `delete-video.e2e.test.ts` create/destroy the fixture** — reusing one upload per target (`"me"` + the test channel, 2 total) across the whole chain instead of the old one-upload-per-file approach (~11 uploads, ~8min). `delete-video.e2e.test.ts` is both the real assertion for `DELETE /api/v1/video/delete/:chatId/:messageId` *and* the cleanup step: its `afterAll` re-attempts `removeFixture` (a thin direct call to `deleteVideoMessage` from `@/telegram-client`, not through the route) wrapped in a silent `try/catch`, so the account never keeps an orphaned fixture even if the delete assertion itself never ran.

`test/e2e/helpers/video-fixture.ts` centralizes `TARGETS` (`"me"`/Saved Messages and `SMOKE_TEST_CHANNEL_ID`, see "Configuration" below), the description constants, `removeFixture`, and `buildSmallThumbnailBuffer()` — the upload fixture's `file` is `src/_assets/sample/15158346_3840_2160_60fps.mp4` (~177MB, 4K/60fps; deliberately much larger than a synthetic buffer so the Range/chunked-streaming tests exercise many real `CHUNK_SIZE` iterations, not a handful of bytes), and its `thumbnail` is `src/_assets/sample/file_example_JPG_1MB.jpg` **resized to ≤320×320 via `sharp`** before being attached — the Telegram API expects a small thumbnail and rejects the original 3800×2534/~1MB file as-is.
...
Because the fixture is large, `stream-video.e2e.test.ts`/`download-video.e2e.test.ts` mostly assert against a small byte-exact `Range` slice (`bytes=0-65535`) compared against the same slice read from the local file, rather than re-downloading the full ~177MB on every assertion; the one full, unranged download (200, full `Content-Length`, full-buffer equality) runs once, only against `"me"` (the faster target), which is enough to prove that code path without paying for it everywhere. Both routes share `streamTelegramVideo` (`src/http/video-stream.ts`) and only differ in `Content-Disposition` (`inline` vs `attachment`); both also get a signed-url-without-`Authorization`-header check (`createSignedUrl` from `@/signed-url`, swapping `/video/stream/` for `/video/dl/` when needed, since the signature covers only `chatId:messageId:exp`, not the path) and a tampered-signature-rejects-with-401 check — this bypass path existed before but had never been exercised end-to-end against a real upload until now.

**Each file must call `client.destroy()` (not `client.disconnect()`) in a file-level `afterAll`** (registered once, outside any `describe.each`, so it runs after all targets in that file — not per target, which would leave the shared `TelegramClient` singleton's `connected` flag stuck `true` after an early disconnect and break the next target's `ensureConnected()`). `disconnect()` alone never sets GramJS's internal `_destroyed` flag, so its background `_updateLoop` keeps waking up every 9s trying to ping the now-disconnected sender indefinitely — that's what produces the `[Error: TIMEOUT]` / `Cannot log after tests are done` noise after a suite finishes, and it's also why the process doesn't exit on its own without `destroy()`. Only `destroy()` sets `_destroyed = true`, letting that loop actually stop.

A full `test:e2e` run now does only 2 real uploads (one per target) instead of ~11, but each is far larger (~177MB vs ~17MB) — expect it to take noticeably longer in absolute time than the old ~8-minute run, dominated by those two uploads. This is a deliberate trade-off, reversing the previous "isolation over speed" stance now that the fixture itself is heavier: fewer, bigger round trips instead of many small independent ones.
