...
describe.each(TARGETS)('GET /api/v1/videos/grouped (e2e) — $label', ({ chatId }) => {
  ...
  beforeAll(async () => {
    ...
    if (!item) throw new Error(`Fixture "${chatId}" não apareceu em /videos/grouped?file_name=${TEST_QUEUE_FILE_NAME}`);
  ...
  it('filters by file_name', async () => {
    ...
    expect(res.body.every((entry: { file_name: string }) => entry.file_name === TEST_QUEUE_FILE_NAME)).toBe(true);
  });
