...
import { removeAudioFixture, TARGETS, TEST_AUDIO_PATH } from './helpers/audio-fixture';
...
describe.each(TARGETS)('DELETE /api/v1/audio/delete/:chatId/:messageId (e2e) — $label', ({ chatId }) => {
  ...
  afterAll(async () => {
    ...
    if (messageId) {
      try {
        await removeAudioFixture(chatId, messageId);
      } catch {
  ...
  it('deletes the fixture and it stops being reachable afterward', async () => {
    const res = await authed(request(app).delete(`/api/v1/audio/delete/${chatId}/${messageId}`));

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ deleted: true, chat_id: chatId, message_id: String(messageId) });

    const streamRes = await authed(request(app).get(`/api/v1/audio/stream/${chatId}/${messageId}`));
    expect(streamRes.status).toBe(404);

    const listRes = await authed(request(app).get(`/api/v1/audios/by/${chatId}`)).query({ limit: 200 });
  ...
  it('returns 200 for a message id that no longer exists (Telegram deleteMessages is idempotent)', async () => {
    const res = await authed(request(app).delete(`/api/v1/audio/delete/${chatId}/999999999`));
    expect(res.status).toBe(200);
